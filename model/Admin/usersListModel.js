...
const usersListModel = {
  ...
  menu_Item_create_v2: async (req, res) => {
    try {

      // formData, link, menu_icon

      // const { title_en, title_bn, link_path, link_path_type, active, parent_id, admin_template_menu_id, menu_icon, icon_align, content_en } = req.body;
      const { formData, link, menu_icon } = req.body;

      const link_path = link && link.value !== '' ? link.value : formData.link_path

      const query = 'INSERT INTO admin_template_menu (title_en, link_path, link_path_type, active,  admin_template_menu_id, menu_icon, icon_align) VALUES (?, ?, ?, ?, ?, ?, ?)';
      connection.query(query, [formData.title_en, link_path, link.type, formData.active, formData.menu_id, menu_icon, formData.floating_position], (error, result) => {
        if (!error) {
          console.log(result);
          return res.send(result);
        } else {
