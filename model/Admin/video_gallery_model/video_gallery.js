...
`;
  ...
  video_gallery_search: async (req, res) => {
    try {
      console.log("Search button clicked.");

      // Extract necessary data from request
      let { statusFilter, fromDate, toDate, videoTitle, categoryFilter } = req.body;
  ...
  LEFT JOIN video_category ON video_gallery.video_category = video_category.id  
      ...
      if (videoTitle) {
        const query = videoTitle.toLowerCase();
        sql += ` AND LOWER(video_gallery.title) LIKE '%${query}%' `;
      }

      // Add status filter condition
      if (statusFilter !== '') {
        sql += ` AND video_gallery.status = ${statusFilter} `;
      }
      if (categoryFilter !== '') {
        sql += ` AND video_gallery.video_category = ${categoryFilter} `;
      }
      ...
      if (fromDate && toDate) {
        ...
        sql += ` AND video_gallery.created_date BETWEEN '${fromDate}' AND '${toDate}' `;
      } else if (fromDate && !toDate) {
        sql += ` AND video_gallery.created_date >= '${fromDate}' `;
      } else if (!fromDate && toDate) {
        sql += ` AND video_gallery.created_date <= '${toDate}' `;
      }
      // Add multiSearch condition

      sql += ` ORDER BY video_gallery.id DESC`; // Append convertedData to the ORDER BY clause
